...
namespace PortalDefender.AavegotchiKit.WearableBrowser
{
    public class WearableBrowserUI : MonoBehaviour
    {
        ...
        void InitTabGroup()
        {
            ...
            {
                if((int)slot != (int)GotchiEquipmentSlot.BG)
        ...
        void Refresh()
        {
            ...
            var nullWerable = new Wearable();
            nullWerable.id = -1;
            var nullItem = Instantiate(itemPrefab, contentRoot);
            nullItem.Init(nullWerable, OnItemSelected);
            items.Add(nullItem.gameObject);
        ...
        {
            Debug.Log("Item Selected"+ item.Data.id);
